						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = m_font->initialize(device, "../ClothSim/Engine/data/fontdata.txt", L"../ClothSim/Engine/data/font.dds");
	if(!result)
	{
...
bool Text::setMousePosition(int mouseX, int mouseY, ID3D11DeviceContext* deviceContext)
{
	...
	result = updateSentence(m_sentence1, mouseString, 40, 50, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	result = updateSentence(m_sentence2, mouseString, 40, 65, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
